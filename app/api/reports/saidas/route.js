...
export async function GET(request) {
  ...
  const rows = futuro
    ? futureRows(lancamentos).filter((r) => r.tipo === "saida")
    : computeLedgerRealizado(records, lancamentos, financas).filter((r) => r.tipo === "saida");
  ...
  const bytes = await buildLedgerPDF({
    church, title: futuro ? "Saídas Futuras (projeção)" : "Relatório de Saídas/Despesas",
    subtitle: `Emitido em ${today()}`, rows, showSaldo: false, me,
  });
  return new Response(bytes, {
    headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="saidas${futuro ? "-futuras" : ""}.pdf"` },
  });
}
