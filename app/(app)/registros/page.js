...
function fmt(d) {
  if(!d) return '-';
  return new Date(d).toLocaleString('pt-BR', { timeZone: 'America/Recife', day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit' })
}
...
export default async function RegistrosPage() {
  ...
  if (isPastor) {
    // PASTOR: SÓ VÊ DEPOIS DE VALIDADO PELO TESOUREIRO
    regsFiltrados = regsAll.filter(r => r.status === 'validado')
  } else if (isTesoureiro) {
    regsFiltrados = regsAll.filter(r => ['aguardando_tesoureiro','validado','devolvido_com_erro'].includes(r.status))
  } else {
    regsFiltrados = regsAll.filter(r => r.primeiro_diacono_id === eu.id || r.segundo_diacono_id === eu.id || r.diacono_id === eu.id)
  }

  const grupos = {}
  regsFiltrados.forEach(r => { const key = r.data_culto || r.created_at?.slice(0,10); if (!grupos[key]) grupos[key] = []; grupos[key].push(r) })

  return (
    <div className="p-4 max-w-3xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="font-bold">Olá {eu.nome} ({eu.oficio}) - {Object.keys(grupos).length} cultos {isPastor? '(só validados)' : ''}</h1>
        {/* TRAVA: Pastor NÃO vê botão Novo Registro */}
        {!isPastor && (
          <a href="/registros/novo" className="bg-green-700 text-white px-4 py-2 rounded font-bold">+ Novo Registro</a>
        )}
        {isPastor && (
          <a href="/registros/novo" className="bg-blue-700 text-white px-4 py-2 rounded font-bold">🔓 Liberar Diáconos</a>
        )}
      </div>

      {Object.keys(grupos).length === 0 && (
        <div className="bg-yellow-50 border p-4 rounded text-center">
          {isPastor? 'Nenhum registro validado pelo tesoureiro ainda.' : 'Nenhum registro encontrado.'}
        </div>
      )}
        ...
        return (
          <div key={dataCulto} className="bg-white p-4 rounded shadow border-l-4 border-l-green-800 space-y-3">
            <div className="flex justify-between font-bold">
              ...
              <span className="text-xs bg-green-100 px-2 py-1 rounded">VALIDADO</span>
            </div>
            ...
            <div className="bg-gray-100 p-3 rounded text-sm font-bold"><p>Dízimo: R$ {totalDizimo.toFixed(2)}</p><p>Ofertas: R$ {totalOferta.toFixed(2)}</p><p className="border-t pt-1">TOTAL: R$ {totalGeral.toFixed(2)}</p></div>
            <div className="bg-blue-50 p-3 rounded text-sm border space-y-1">
              <p className="font-bold">Histórico:</p>
              {(primeiro.historico || []).map((h,i) => (<p key={i}>• {fmt(h.em)} - <b>{h.usuario || h.usuario_nome}</b> - {h.acao}</p>))}
            </div>
