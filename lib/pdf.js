...
export async function buildBalancetePDF({ church, saldoInicial, totalEntradas, totalSaidas, categoriasSaida, periodo, me }) {
  const c = await createPDFCursor();
  await c.setLogo(church.logo);
  c.title("Balancete");
  c.header(church, me);
  ...
  c.line(`(+) Total de entradas no periodo: ${brl(totalEntradas)}`);
  c.line(`(-) Total de saidas no periodo: ${brl(totalSaidas)}`);
  c.space(10);
  c.subtitle("Saídas por categoria");
  const entries = Object.entries(categoriasSaida);
  if (entries.length === 0) c.line("Nenhuma saída no período.", { size: 9.5 });
  else entries.forEach(([cat, val]) => c.line(`${cat}: ${brl(val)}`, { size: 9.5 }));
  c.hr();
...
export async function buildReciboDizimistaPDF({ church, nomeDizimista, itens, periodo, tesoureiro, me }) {
  const c = await createPDFCursor();
  await c.setLogo(church.logo);
  c.title("Recibo de Dizimista");
  c.header(church, me);
  c.line(periodo, { size: 9 });
  c.subtitle(`Dizimista: ${nomeDizimista}`);
  const total = itens.reduce((s, i) => s + Number(i.valor), 0);
  if (itens.length === 0) c.line("Nenhum dízimo registrado no período.");
  else c.table([{ label: "Data", x: MARGIN }, { label: "Valor", x: MARGIN + 200 }], itens.map((i) => [fmtDate(i.data), brl(i.valor)]));
  c.hr();
  c.line(`Total dizimado no período: ${brl(total)}`, { bold: true, size: 11 });
