...
export default function FormRelatorio({ eu, registros = [], igreja }){
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-white border p-4 rounded max-w-5xl mt-4">
      ...
      {gerou && (
        <div className="bg-white border mt-6 rounded max-w-5xl overflow-hidden">
          <div className="flex gap-4 p-4 border-b-2 border-[#1a4330]">
            <img src={dadosIgreja.logo} className="h-[70px]" onError={e=>e.target.style.display='none'} alt="logo" />
            <div>
              ...
              <div className="text-[11px] text-gray-500 mt-2">Emitido em: {new Date().toLocaleString('pt-BR')} · Por: {eu?.nome} ({eu?.oficio})</div>
            </div>
          </div>
          <div className="p-4">
            ...
            <div className="border-t-2 border-[#1a4330] pt-3 mt-4 font-bold">Dízimos: R$ {totalDizimo.toFixed(2)} · Ofertas: R$ {totalOferta.toFixed(2)} · Total geral: R$ {(totalDizimo+totalOferta).toFixed(2)}</div>
          </div>
        </div>
      )}
    </div>
