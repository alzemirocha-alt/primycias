...
export default function AvisosBoard({ me, avisos }) {
  const [titulo, setTitulo] = useState(""); const [mensagem, setMensagem] = useState("");
  const [dataEvento, setDataEvento] = useState(""); const [horaEvento, setHoraEvento] = useState("");
  const [linkUrl, setLinkUrl] = useState(""); const [videoUrl, setVideoUrl] = useState("");
  const [imagemUrl, setImagemUrl] = useState(""); const [arquivoUrl, setArquivoUrl] = useState("");
  const [integrar, setIntegrar] = useState(true); const [loading, setLoading] = useState(false);
  ...
  async function uploadArquivo(e) {
    ...
    const { url } = await res.json(); setArquivoUrl(url);
    if (file.type.startsWith("image/")) setImagemUrl(url);
  }
  ...
  async function excluir(id) { if (!confirm("Excluir aviso?")) return; await fetch(`/api/avisos?id=${id}`, { method: "DELETE" }); location.reload(); }

  function iniciarEdicao(a) {
    setEditando(a.id); setTitulo(a.titulo); setMensagem(a.mensagem || a.conteudo); setImagemUrl(a.imagem_url || ""); setVideoUrl(a.video_url || ""); setLinkUrl(a.link_url || ""); setArquivoUrl(a.arquivo_url || "");
    ...
    <div className="bg-white border border-line rounded-sm p-4 mb-6">
      ...
      {podePostar && (
        <div className="border border-paperDeep p-3 rounded-sm mb-4 bg-[#faf9f6]">
          ...
          <div className="grid grid-cols-2 gap-2 mb-2"><input type="date" value={dataEvento} onChange={e=>setDataEvento(e.target.value)} className="border p-2 text-sm" /><input type="time" value={horaEvento} onChange={e=>setHoraEvento(e.target.value)} className="border p-2 text-sm" /></div>
          <div className="grid gap-2 mb-2">
            ...
            <div className="flex items-center gap-2"><label className="text-xs bg-white border px-3 py-2 cursor-pointer">📎 Anexar arquivo<input type="file" hidden onChange={uploadArquivo} /></label>{arquivoUrl && <span className="text-xs text-green-700 truncate">{arquivoUrl}</span>}</div>
          </div>
          ...
          <div className="flex gap-2"><button onClick={salvar} disabled={loading} className="bg-[#0F3A1F] text-white px-4 py-2 text-sm">{loading? "..." : editando? "Salvar Edição" : "Publicar Aviso"}</button>{editando && <button onClick={()=>{setEditando(null); setTitulo(""); setMensagem("")}} className="border px-4 py-2 text-sm">Cancelar</button>}</div>
        </div>
      )}
      <div className="space-y-3">
        {avisos?.map(a => (
          <div key={a.id} className="border-b pb-3">
            <div className="flex justify-between"><span className="font-medium text-sm">{a.titulo} {a.data_evento && <span className="text-xs text-gray-500">- {new Date(a.data_evento).toLocaleString('pt-BR')}</span>}</span>{podePostar && <span className="flex gap-2"><button onClick={()=>iniciarEdicao(a)} className="text-xs text-blue-600">Editar</button><button onClick={()=>excluir(a.id)} className="text-xs text-red-600">Excluir</button></span>}</div>
            ...
            {a.arquivo_url &&!a.imagem_url && <a href={a.arquivo_url} target="_blank" className="text-xs text-blue-600 underline mt-1 block">📎 Baixar anexo</a>}
